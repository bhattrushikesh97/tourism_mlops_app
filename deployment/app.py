...
# CONFIG
# =========================
HF_TOKEN = os.getenv("HF_TOKEN")  # optional if private repo
...
def load_model():
    try:
        model_path = hf_hub_download(
            repo_id=MODEL_REPO,
            filename=MODEL_FILE,
            token=HF_TOKEN  # works for both public & private
        )
...
# =========================
st.set_page_config(
    page_title="Tourism_Package_Prediction",
    page_icon="✈️",
    layout="wide"
)
...
with col2:
    ...
    MonthlyIncome = st.number_input(
        "Monthly Income", 1000, 500000, 30000
    )
...
if st.button("Predict"):
    ...
    if model is None:
        ...
        try:
            ...
            if prediction == 1:
                st.success("✅ Customer is  purchase the package")
            else:
                st.error("❌ Customer is not purchase the package")
